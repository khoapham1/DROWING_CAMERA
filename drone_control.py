...
try:
    ...
    USE_UNDISTORT = True
    print("âœ… Loaded camera calibration matrices")
except Exception as e:
    CAMERA_MATRIX = None
    DIST_COEFF = None
    USE_UNDISTORT = False
    print("âš ï¸ Camera calibration not loaded:", e)
...
def start_camera(camera_index=0):
    ...
    try:
        ...
        print("âœ… USB camera started")
    except Exception as e:
        print("âŒ Failed to start USB camera:", e)
def _camera_loop():
        ...
        try:
            ...
            if USE_UNDISTORT and CAMERA_MATRIX is not None:
                if _undistort_map1 is None:
                    ...
                    print("âœ… Undistort map initialized")
                frame = cv2.remap(
                    frame,
                    _undistort_map1,
                    _undistort_map2,
...
class DroneController:
    def __init__(self, connection_str='/dev/ttyACM0', takeoff_height=4):
        ...
        try:
            ...
            print("âœ… Vehicle connected successfully")
        except Exception as e:
            print(f"âŒ Failed to connect to vehicle: {e}")
            self.vehicle = None
        # Telemetry buffer
        ...
        if self.vehicle:
            try:
                ...
                print("âœ… Listeners and parameters set")
            except Exception as e:
...
# -------- Telemetry listeners --------
    ...
    def start_image_stream(self):
        ...
        try:
            ...
            print("âŒ Failed to start camera:", e)
    ...
    def start_person_detection(self):
        ...
        print("âœ… Person detection started")
    # ------- MEDIAPIPE --------
    ...
    def draw_skeleton(self, frame, pose_landmarks, color):
        ...
        if pose_landmarks:
            ...
            # Váº½ Ä‘iá»ƒm Ä‘áº§u (nose)
            h, w = frame.shape[:2]
            nose = pose_landmarks.landmark[0]
            nose_x, nose_y = int(nose.x * w), int(nose.y * h)
            cv2.circle(frame, (nose_x, nose_y), 3, (255, 0, 0), -1)
    def _person_detection_loop(self):
        """Person + Drowning detection loop (YOLO each frame, MediaPipe maintains presence)."""
        from types import SimpleNamespace

        # Virtual track used when YOLO misses but MediaPipe Pose is still tracking a person
        if not hasattr(self, "_mp_track"):
            self._mp_track = SimpleNamespace(
                id=-1,
                bbox=[0, 0, 0, 0],
                miss=0,
                pose_history=[],
        ...
        if not hasattr(self, "_mp_last_bbox"):
            self._mp_last_bbox = None
        ...
        while self.person_running:
            try:
                ...
                # If streamer returns the exact same object repeatedly, skip re-processing
                if frame_jpeg is last_frame_obj:
                    time.sleep(0.002)
                    continue
                last_frame_obj = frame_jpeg
                ...
                # --- MediaPipe Pose every frame (tracking/persistence) ---
                ...
                # Decide "person present" by Pose (for realtime maintain)
                mp_person = False
                mp_bbox = None
                if pose_lm:
                    ...
                    vis_cnt = 0
                    for lm in lms:
                        if float(getattr(lm, "visibility", 1.0)) < vis_th:
                            continue
                        x = float(lm.x)
                        y = float(lm.y)
                    ...
                    # Require enough visible points to reduce false positives
                    mp_person = (vis_cnt >= 8)

                    if mp_person and xs and ys:
                        x1 = min(xs) * w
                        x2 = max(xs) * w
                        y1 = min(ys) * h
                        y2 = max(ys) * h
                ...
                # fallback bbox for Pose-only frames
                if mp_person and mp_bbox is None:
                    mp_bbox = self._mp_last_bbox
                if mp_bbox is not None:
                    self._mp_last_bbox = mp_bbox

                # --- YOLO each frame (do NOT reuse old frames/tracks) ---
                # Force detector to run every frame
                try:
                    if hasattr(self.person_detector, "detect_interval"):
                        self.person_detector.detect_interval = 1
                except Exception:
                    pass

                yolo_raw = self.person_detector.detect(cv_image) or []

                # Remove tracker-persistent tracks (carry-over): keep only tracks updated THIS frame
                detections = []
                for d in yolo_raw:
                    ...
                    if t is None:
                        continue
                    try:
                        if int(getattr(t, "miss", 0)) != 0:
                            continue
                    except Exception:
                        pass
                    detections.append(d)

                # If YOLO misses but Pose still sees a person -> virtual detection from MediaPipe
                if (not detections) and mp_person and (mp_bbox is not None):
                    t = self._mp_track
                    try:
                        t.bbox = mp_bbox
                        t.miss = 0
                ...
                # --- Drowning detection + overlay ---
                states = {"ACTIVE": 0, "FROZEN": 0, "SOS": 0}
                for det in detections:
                    track = det["track_obj"]
                    bbox = det["bbox"]

                    results = self.drowing_detector.detect(track, bbox, pose_lm, (h, w))
                    if results.get("state") == "SOS":
                        results["sos_duration"] = self.drowing_detector.get_sos_duration(track)

                    det["drowning_state"] = results
                    st = results.get("state", "ACTIVE")
                    states[st] = states.get(st, 0) + 1

                    color = self.draw_results(cv_image, bbox, results)
                    if pose_lm:
                        self.draw_skeleton(cv_image, pose_lm, color)
                ...
                # Keep 1 detection for mission/server logic
                ...
                # small pacing (keeps CPU reasonable but still realtime)
                time.sleep(0.002)
    ...
    def send_person_detection_to_server(self, lat, lon, alt, detections):
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"âœ… Sent {len(detections)} person detections to server")
            else:
                print(f"âŒ Failed to send detections: {response.status_code}")
    ...
    def set_speed(self, speed):
        ...
        self.vehicle.send_mavlink(msg)
        self.vehicle.flush()
        print(f"âœ… Speed set to {speed} m/s")
    def set_fixed_heading(self, heading_deg, yaw_rate=10, relative=False):
        ...
        self.vehicle.send_mavlink(msg)
        self.vehicle.flush()
        print(f"âœ… Heading set to {heading_deg}Â°")
    # -------- Mission control --------
    def arm_and_takeoff(self, targetHeight):
        ...
        while True:
            ...
            print(f'ðŸ“Š Altitude: {alt:.2f}' if alt else 'Altitude: 0.00')
